...
class BPE(object):
    ...
    def segment_tokens(self, tokens):
        ...
        for word in tokens:
            ...
            if not word:
                continue
            new_word = [
                out for segment in self._isolate_glossaries(word) for out in encode(
                    segment, self.bpe_codes, self.bpe_codes_reverse, self.vocab, self.separator, self.version,
                    self.cache, self.glossaries
                )
            ]
    ...
    def _isolate_glossaries(self, word):
        ...
        for gloss in self.glossaries:
            word_segments = [
                out_segments for segment in word_segments for out_segments in isolate_glossary(segment, gloss)
            ]
